...
- [ ] **MCU decoupling (§5, §14).** A close-in 100 nF at each supply pin the part bonds on UFQFPN48: VDD (24, 36), VBAT (1), NRST (7), plus the VDD bulk reservoir. VCAP_1 (pin 22) carries exactly one low-ESR ceramic and VCAP_2 is absent, so a second core cap must not be added. VDDA/VREF+ (pin 9) is filtered from VDD through a ferrite, not regulated: no on-chip ADC is used.
