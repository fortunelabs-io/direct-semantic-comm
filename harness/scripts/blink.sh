...
# once and fail on every run afterwards. The build directory is excluded
# for the same reason: it is generated by this gate and is gitignored, but
# the exclusion is stated rather than relied upon.
tree_dirty() {
    git -C "${REPO_ROOT}" status --porcelain \
        -- ":(exclude)harness/results/$(basename "${RESULTS_FILE}")" \
           ":(exclude)harness/firmware/capture/build"
}
